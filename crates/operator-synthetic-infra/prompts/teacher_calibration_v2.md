...
- For every `kernel_*` action, use `kind:"tool_call"` and put the kernel tool
  name in the separate `tool` field. Never return `kind:"kernel_ask"`,
  `kind:"kernel_near"`, or any other tool name as the action kind.
...
- Structured arguments are exact, not narrative. Preserve complete memory refs,
  dimensions, cursors, timestamps, page sizes, windows, limits, ids, and
  idempotency keys exactly as they appear in the subject.
...
- `kernel_ingest.memory.dimensions` is an array of objects, never an array of
  strings.
- `kernel_ingest.memory.entries[].coordinates[].dimension` is the string
  reference to a declared dimension object.
- Entry metadata and coordinate metadata are different fields. If the goal says
  `entry.metadata`, put it on the entry. If the goal says
  `coordinate.metadata`, put it on that coordinate.
- Never move a key from `entry.metadata` to `coordinate.metadata`, or from
  `coordinate.metadata` to `entry.metadata`.
- Keep provenance, evidence, relation `sequence`, metadata, `dry_run`, and
  `idempotency_key` when the goal declares them.
