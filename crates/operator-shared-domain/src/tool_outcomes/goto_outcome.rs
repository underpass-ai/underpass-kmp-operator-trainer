//! Typed projection of `kernel_goto` structured content. First-pass
//! shape mirrors `NearOutcome`: a summary plus the entries the kernel
//! resolved at the requested cursor.
...
mod tests {
    ...
    fn exposes_summary_and_entries() {
        let outcome = GotoOutcome::new(
            NonEmptyString::parse("goto summary", "goto_outcome.summary").unwrap(),
            vec![MemoryRef::parse("entry:1").unwrap()],
        );
        assert_eq!(outcome.summary().as_str(), "goto summary");
